...
class serverConnection:
    def __init__(self):
        ...
        self.sock.connect(("nitrogen.student.utwente.nl",3636))
        data = self.RSALocal.publickey().exportKey("PEM")
        self.sock.sendall(data)
    def transact(self,f, t, amount, seed):
        data = "%s,%s,%s,%s" % (f,t,amount,seed)
        msg = self.RSAServer.encrypt(data.encode("utf8"), None)
        self.sock.sendall(msg[0])
        print(self.RSALocal.decrypt(self.sock.recv(4096)))
